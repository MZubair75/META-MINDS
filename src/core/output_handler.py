    ...
    try:
        ...
        if output_dir and not os.path.exists(output_dir):
            try:
                ...
                logging.warning(f"Permission denied creating directory {output_dir}: {e}. Using current directory.")
...
def save_separate_reports(dataset_summaries: Dict[str, Any], 
    ...
    if not os.path.exists(output_dir):
        try:
            ...
            print(f"Warning: Could not create {output_dir} directory. Using current directory.")
            output_dir = "."
    
    # If Output directory exists but we can't write to it, use current directory
    if os.path.exists(output_dir) and not os.access(output_dir, os.W_OK):
        print(f"Warning: Cannot write to {output_dir} directory. Using current directory.")
        output_dir = "."
